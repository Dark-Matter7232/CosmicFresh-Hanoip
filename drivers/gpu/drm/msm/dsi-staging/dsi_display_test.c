...
static void dsi_display_test_work(struct work_struct *work)
{
	...
	if (rc) {
		pr_debug_once("failed to get modes count, rc=%d\n", rc);
		goto test_fail;
	}
	...
	if (rc) {
		pr_debug_once("failed to set mode, rc=%d\n", rc);
		goto test_fail_free_modes;
	}

	rc = dsi_display_prepare(display);
	if (rc) {
		pr_debug_once("failed to prepare display, rc=%d\n", rc);
		goto test_fail_free_modes;
	}

	rc = dsi_display_enable(display);
	if (rc) {
		pr_debug_once("failed to enable display, rc=%d\n", rc);
...
test_fail_unprep_disp:
	if (rc) {
		pr_debug_once("failed to unprep display, rc=%d\n", rc);
		goto test_fail_free_modes;
	}
