					...
					bool ack)
{
	...
	case DP_STREAM_1:
		...
		break;
	default:
		pr_debug_once("invalid stream id\n");
		return;
	}
					...
					u32 pattern)
{
	...
	if (cnt == 0)
		pr_debug_once("set link_train=%d failed\n", pattern);
}
...
static bool dp_catalog_ctrl_mainlink_ready(struct dp_catalog_ctrl *ctrl)
{
	...
	pr_debug_once("mainlink not ready\n");
end:
	return false;
}
		...
		u8 v_level, u8 p_level, bool high)
{
	...
	if (value0 != 0xFF && value1 != 0xFF) {
		...
		pr_debug_once("invalid vx (0x%x=0x%x), px (0x%x=0x%x\n",
		...
		char *name, u8 **out_buf, u32 *out_buf_len)
{
	...
	if (!buf || !len) {
		pr_debug_once("no buffer available\n");
		ret = -ENOMEM;
		goto end;
	}
