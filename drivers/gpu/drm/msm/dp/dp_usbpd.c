		...
		bool peer_usb_comm)
{
	...
	if (!pd) {
		pr_debug_once("get_usbpd phandle failed\n");
		return;
	}
	...
	enum usbpd_svdm_cmd_type cmd_type, int num_vdos)
{
	...
	if (cmd_type == SVDM_CMD_TYPE_RESP_NAK) {
		pr_debug_once("error: NACK\n");
		ret = -EINVAL;
		goto end;
	}

	if (cmd_type == SVDM_CMD_TYPE_RESP_BUSY) {
		pr_debug_once("error: BUSY\n");
		ret = -EBUSY;
		goto end;
	}
	...
	if (cmd == USBPD_SVDM_ATTENTION) {
		if (cmd_type != SVDM_CMD_TYPE_INITIATOR) {
			pr_debug_once("error: invalid cmd type for attention\n");
			ret = -EINVAL;
			goto end;
		}

		if (!num_vdos) {
			pr_debug_once("error: no vdo provided\n");
			ret = -EINVAL;
			goto end;
		}
	} else {
		if (cmd_type != SVDM_CMD_TYPE_RESP_ACK) {
			pr_debug_once("error: invalid cmd type\n");
			ret = -EINVAL;
		}
	}
end:
				...
				const u32 *vdos, int num_vdos)
{
	...
	case DP_USBPD_VDM_CONFIGURE:
		...
		if (rc) {
			pr_debug_once("failed to get SuperSpeed lanes\n");
			break;
		}
...
int dp_usbpd_register(struct dp_hpd *dp_hpd)
{
	...
	if (rc)
		pr_debug_once("pd registration failed\n");
...
static void dp_usbpd_wakeup_phy(struct dp_hpd *dp_hpd, bool wakeup)
{
	...
	if (!usbpd->pd) {
		pr_debug_once("usbpd pointer invalid");
		return;
	}
