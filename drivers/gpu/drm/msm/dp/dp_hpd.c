		...
		struct dp_hpd_cb *cb)
{
	...
	if (aux_bridge && (aux_bridge->flag & MSM_DP_AUX_BRIDGE_HPD)) {
		...
		if (IS_ERR(dp_hpd)) {
			pr_debug_once("failed to get bridge hpd\n");
			goto out;
		}
		...
		if (IS_ERR(dp_hpd)) {
			pr_debug_once("failed to get lphw hpd\n");
			return dp_hpd;
		}
		...
		if (IS_ERR(dp_hpd)) {
			pr_debug_once("failed to get gpio hpd\n");
			goto out;
		}
		...
		if (IS_ERR(dp_hpd)) {
			pr_debug_once("failed to get usbpd\n");
			goto out;
		}
...
void dp_hpd_put(struct dp_hpd *dp_hpd)
{
	...
	case DP_HPD_BRIDGE:
		...
		break;
	default:
		pr_debug_once("unknown hpd type %d\n", dp_hpd->type);
		break;
	}
}
