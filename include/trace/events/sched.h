...
TRACE_EVENT(sched_load_balance,
	...
	TP_ARGS(cpu, idle, balance, group_mask, busiest_nr_running,
		imbalance, env_flags, ld_moved, balance_interval, active_balance, prefer_spread),
...
TRACE_EVENT(sched_task_util,
	...
	TP_printk("pid=%d comm=%s util=%lu prev_cpu=%d next_cpu=%d backup_cpu=%d target_cpu=%d sync=%d need_idle=%d fastpath=%d placement_boost=%d latency=%llu stune_boosted=%d is_rtg=%d rtg_skip_min=%d start_cpu=%d unfilter=%u low_latency=%d",
		...
		__entry->stune_boosted, __entry->is_rtg, __entry->rtg_skip_min, 
		__entry->start_cpu, __entry->unfilter, __entry->low_latency)
)
