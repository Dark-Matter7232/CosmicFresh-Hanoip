...
static void configure_mem(void) {
	struct sysinfo i;
	si_meminfo(&i);
	if (i.totalram > 4096ull * 1024 * 1024) {
	linux_sh("/system/bin/echo 2952790016 > /sys/block/zram0/disksize");
	} else {
	linux_sh("/system/bin/echo 1908408320 > /sys/block/zram0/disksize");
	linux_sh("/system/bin/echo 128 > /sys/module/lowmemorykiller/parameters/slmk_minfree");
	}
}
