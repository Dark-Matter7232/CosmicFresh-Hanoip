...
static inline int dp_hdcp2p2_valid_handle(struct dp_hdcp2p2_ctrl *ctrl)
{
	...
	if (!ctrl->lib_ctx) {
		pr_debug_once("HDCP library needs to be acquired\n");
		return -EINVAL;
	}

	if (!ctrl->lib) {
		pr_debug_once("invalid lib ops data\n");
		return -EINVAL;
	}
	return 0;
}
...
static int dp_hdcp2p2_wakeup(struct hdcp_transport_wakeup_data *data)
{
	...
	case HDCP_TRANSPORT_CMD_AUTHENTICATE:
		...
		break;
	default:
		pr_debug_once("invalid wakeup command %d\n", ctrl->wakeup_cmd);
	}
exit:
	...
	struct sde_hdcp_2x_wakeup_data *data)
{
	...
	if (ctrl && ctrl->lib && ctrl->lib->wakeup &&
		...
		if (rc)
			pr_debug_once("error sending %s to lib\n",
...
static int dp_hdcp2p2_on(void *input)
{
	...
	rc = ctrl->lib->wakeup(&cdata);
	if (rc)
		pr_debug_once("Unable to start the HDCP 2.2 library. Error - %d", rc);
...
static int dp_hdcp2p2_aux_read_message(struct dp_hdcp2p2_ctrl *ctrl)
{
	...
	if (atomic_read(&ctrl->auth_state) == HDCP_STATE_INACTIVE ||
		...
		pr_debug_once("invalid hdcp state\n");
		rc = -EINVAL;
		goto exit;
	}

	if (!buf) {
		pr_debug_once("invalid request buffer\n");
		rc = -EINVAL;
		goto exit;
	}
...
static void dp_hdcp2p2_send_msg_work(struct kthread_work *work)
{
	...
	rc = dp_hdcp2p2_aux_write_message(ctrl, ctrl->response.data,
			...
			ctrl->timeout);
	if (rc) {
		pr_debug_once("Error sending msg to sink %d\n", rc);
		mutex_unlock(&ctrl->msg_lock);
		goto exit;
	}
...
static int dp_hdcp2p2_get_msg_from_sink(struct dp_hdcp2p2_ctrl *ctrl)
{
	...
	if (rc) {
		pr_debug_once("error reading message %d\n", rc);
		goto exit;
	}
...
static void dp_hdcp2p2_link_work(struct kthread_work *work)
{
	...
	if (ctrl->sink_rx_status & ctrl->abort_mask) {
		if (ctrl->sink_rx_status & BIT(3))
			pr_debug_once("reauth_req set by sink\n");

		if (ctrl->sink_rx_status & BIT(4))
			pr_debug_once("link failure reported by sink\n");
		...
		u8 *rx_status)
{
	...
	*rx_status = 0;
	...
	if (bytes_read != bytes_to_read) {
		pr_debug_once("cp irq read failed\n");
		rc = bytes_read;
		goto error;
	}
	...
	if (cp_irq) {
		...
		if (bytes_read != bytes_to_read) {
			pr_debug_once("rxstatus read failed\n");
			rc = bytes_read;
			goto error;
		}
		*rx_status = buf;
...
static int dp_hdcp2p2_cp_irq(void *input)
{
	...
	if (rc) {
		pr_debug_once("failed to read rx status\n");
		return rc;
	}
...
static bool dp_hdcp2p2_supported(void *input)
{
	...
	if (bytes_read != DP_HDCP_RXCAPS_LENGTH) {
		pr_debug_once("RxCaps read failed\n");
		goto error;
	}
...
void *sde_dp_hdcp2p2_init(struct sde_hdcp_init_data *init_data)
{
	...
	if (rc) {
		pr_debug_once("Unable to register with HDCP 2.2 library\n");
		goto error;
	}
	...
	if (IS_ERR(ctrl->thread)) {
		pr_debug_once("unable to start DP hdcp2p2 thread\n");
