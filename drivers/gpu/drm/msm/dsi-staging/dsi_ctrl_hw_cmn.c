...
void dsi_ctrl_hw_cmn_debug_bus(struct dsi_ctrl_hw *ctrl, u32 *entries, u32 size)
{
	...
	for (i = 0; i < size; i++) {
		...
		pr_debug_once("[DSI_%d] debug bus ctrl: 0x%x status:0x%x\n",
				     ...
				     u32 *hw_read_cnt)
{
	...
	if (ack_err)
		read_cnt -= 4;
	if (!read_cnt) {
		pr_debug_once("Panel detected error, no data read\n");
		return 0;
	}
...
int dsi_ctrl_hw_cmn_wait_for_cmd_mode_mdp_idle(struct dsi_ctrl_hw *ctrl)
{
	...
	if (rc)
		pr_debug_once("%s: timed out waiting for idle\n", __func__);
...
int dsi_ctrl_hw_cmn_wait4dynamic_refresh_done(struct dsi_ctrl_hw *ctrl)
{
	...
	if (rc) {
		pr_debug_once("wait4dynamic refresh timedout %d\n", rc);
		return rc;
	}
