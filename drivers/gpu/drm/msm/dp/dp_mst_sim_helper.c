		...
		struct drm_dp_aux_msg *aux_msg)
{
	...
	if (!msg->have_somt) {
		...
		if (!ret) {
			pr_debug_once("sideband hdr build failed\n");
			return false;
		}
	} else {
		ret = drm_dp_sideband_msg_build(msg,
		...
		if (!ret) {
			pr_debug_once("sideband msg build failed\n");
			return false;
		}
	}
		...
		struct msm_dp_mst_sim_context *ctx)
{
	...
	if (i2c_read.num_transactions == 1) {
		if (i2c_read.transactions[0].i2c_dev_id != DDC_ADDR ||
			...
			pr_debug_once("unsupported i2c address\n");
			goto err;
		}
		...
		pr_debug_once("unsupported i2c transaction\n");
		goto err;
	}
	...
	if (start + len > port->edid_size) {
		pr_debug_once("edid data exceeds maximum\n");
		goto err;
	}
		...
		struct msm_dp_mst_sim_context *ctx)
{
	...
	if (port_num >= ctx->port_num) {
		pr_debug_once("invalid port num\n");
		goto err;
	}
