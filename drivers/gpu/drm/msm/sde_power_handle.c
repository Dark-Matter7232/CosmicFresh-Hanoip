	...
	struct sde_power_handle *phandle, char *client_name)
{
	...
	if (!client_name || !phandle) {
		pr_debug_once("client name is null or invalid power data\n");
...
{
	if (!client  || !phandle) {
		pr_debug_once("reg bus vote: invalid client handle\n");
	} else if (!client->active) {
		pr_debug_once("sde power deinit already done\n");
		kfree(client);
	} else {
				...
				struct dss_module_power *mp)
{
	...
	if (!pdev || !mp) {
		pr_debug_once("invalid input param pdev:%pK mp:%pK\n", pdev, mp);
		return -EINVAL;
	}
		...
		u64 ib_quota_rt, u64 ib_quota_nrt)
{
	...
	if (pdbus->data_bus_hdl < 1) {
		pr_debug_once("invalid bus handle %d\n", pdbus->data_bus_hdl);
		return -EINVAL;
	}
		...
		u64 ab_quota, u64 ib_quota)
{
	...
	if (!phandle || !pclient ||
		...
		pr_debug_once("invalid parameters\n");
		return -EINVAL;
	}
	...
	struct sde_power_data_bus_handle *pdbus, const char *name)
{
	...
	if (node) {
		...
		if (rc) {
			pr_debug_once("Error. qcom,msm-bus,num-paths not found\n");
			return rc;
		}
		...
		if (IS_ERR_OR_NULL(pdbus->data_bus_scale_table)) {
			pr_debug_once("reg bus handle parsing failed\n");
		...
		pdbus->data_bus_hdl = msm_bus_scale_register_client(
				pdbus->data_bus_scale_table);
		if (!pdbus->data_bus_hdl) {
			pr_debug_once("data_bus_client register failed\n");
			rc = -EINVAL;
			goto end;
		}
	...
	struct sde_power_handle *phandle)
{
	...
	if (node) {
		...
		if (!phandle->reg_bus_hdl) {
			pr_debug_once("reg_bus_client register failed\n");
			rc = -EINVAL;
			goto end;
		}
							...
							bool enable)
{
	...
	if (rc)
		pr_debug_once("failed to set data bus vote rc=%d enable:%d\n",
							rc, enable);
...
static int sde_power_reg_bus_update(u32 reg_bus_hdl, u32 usecase_ndx)
{
	...
	if (rc)
		pr_debug_once("failed to set reg bus vote rc=%d\n", rc);
	...
	struct sde_power_handle *phandle)
{
	...
	if (!phandle || !pdev) {
		pr_debug_once("invalid input param\n");
		rc = -EINVAL;
		goto end;
	}
	mp = &phandle->mp;
	...
	if (rc) {
		pr_debug_once("device clock parsing failed\n");
		goto end;
	}

	rc = sde_power_parse_dt_supply(pdev, mp);
	if (rc) {
		pr_debug_once("device vreg supply parsing failed\n");
		goto parse_vreg_err;
	}
	...
	if (rc) {
		pr_debug_once("vreg config failed rc=%d\n", rc);
		goto vreg_err;
	}

	rc = msm_dss_get_clk(&pdev->dev, mp->clk_config, mp->num_clk);
	if (rc) {
		pr_debug_once("clock get failed rc=%d\n", rc);
		goto clk_err;
	}

	rc = msm_dss_clk_set_rate(mp->clk_config, mp->num_clk);
	if (rc) {
		pr_debug_once("clock set rate failed rc=%d\n", rc);
		goto bus_err;
	}

	rc = sde_power_reg_bus_parse(pdev, phandle);
	if (rc) {
		pr_debug_once("register bus parse failed rc=%d\n", rc);
		goto bus_err;
	}
	...
	for (i = SDE_POWER_HANDLE_DBUS_ID_MNOC;
		...
		rc = sde_power_data_bus_parse(pdev,
				...
				data_bus_name[i]);
		if (rc) {
			pr_debug_once("register data bus parse failed id=%d rc=%d\n",
					i, rc);
			goto data_bus_err;
		}
	}
	...
	struct sde_power_handle *phandle)
{
	...
	list_for_each_entry_safe(curr_client, next_client,
		...
		pr_debug_once("cliend:%s-%d still registered with refcount:%d\n",
	...
	list_for_each_entry_safe(curr_event, next_event,
		...
		pr_debug_once("event:%d, client:%s still registered\n",
	...
	struct sde_power_client *pclient, bool enable)
{
	...
	if (enable) {
		...
		for (i = 0; i < SDE_POWER_HANDLE_DBUS_ID_MAX; i++) {
			...
			if (rc) {
				pr_debug_once("failed to set data bus vote id=%d rc=%d\n",
						i, rc);
		...
		if (rc) {
			pr_debug_once("failed to update rsc\n");
			goto rsc_err;
		}
	...
	u64 rate)
{
	...
	if (!phandle) {
		pr_debug_once("invalid input power handle\n");
		return -EINVAL;
	}
	mp = &phandle->mp;
		...
		char *clock_name, unsigned long flags)
{
	...
	if (!clock_name) {
		pr_debug_once("invalid input clock name\n");
		return -EINVAL;
	}

	clk = sde_power_clk_get_clk(phandle, clock_name);
	if (!clk) {
		pr_debug_once("get_clk failed for clk: %s\n", clock_name);
		return -EINVAL;
	}
		...
		void *usr, char *client_name)
{
	...
	if (!phandle) {
		pr_debug_once("invalid power handle\n");
		return ERR_PTR(-EINVAL);
	} else if (!cb_fnc || !event_type) {
		pr_debug_once("no callback fnc or event type\n");
		return ERR_PTR(-EINVAL);
	}
...
{
	if (!phandle || !event) {
		pr_debug_once("invalid phandle or event\n");
	} else if (!event->active) {
		pr_debug_once("power handle deinit already done\n");
		kfree(event);
	} else {
		mutex_lock(&phandle->phandle_lock);
