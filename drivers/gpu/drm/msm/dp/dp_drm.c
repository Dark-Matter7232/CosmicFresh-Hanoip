...
static void dp_bridge_enable(struct drm_bridge *drm_bridge)
{
	...
	if (rc)
		pr_debug_once("[%d] DP display post enable failed, rc=%d\n",
		       bridge->id, rc);
}
...
static void dp_bridge_post_disable(struct drm_bridge *drm_bridge)
{
	...
	if (rc) {
		pr_debug_once("[%d] DP display disable failed, rc=%d\n",
		       bridge->id, rc);
		return;
	}
	...
	if (rc) {
		pr_debug_once("[%d] DP display unprepare failed, rc=%d\n",
		       bridge->id, rc);
		return;
	}
}
	...
	struct sde_connector_state *c_state)
{
	...
	if (!sde_conn->drv_panel) {
		pr_debug_once("invalid dp panel\n");
		return -EINVAL;
	}
		...
		u32 max_mixer_width, void *display)
{
	...
	rc = msm_get_mixer_count(priv, drm_mode, max_mixer_width,
			&topology->num_lm);
	if (rc) {
		pr_debug_once("error getting mixer count, rc:%d\n", rc);
		return rc;
	}
		...
		void *display)
{
	...
	if (dp->is_sst_connected) {
		...
		if (!rc)
			pr_debug_once("failed to get DP sink modes, rc=%d\n", rc);
		...
		pr_debug_once("No sink connected\n");
	}
	kfree(dp_mode);
...
int dp_drm_bridge_init(void *data, struct drm_encoder *encoder)
{
	...
	if (rc) {
		pr_debug_once("request_irq failed, rc=%d\n", rc);
		goto error_free_bridge;
	}
