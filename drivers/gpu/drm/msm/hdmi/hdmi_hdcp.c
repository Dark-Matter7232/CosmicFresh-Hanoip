	...
	u8 *data, u16 data_len)
{
	...
	if (data_len > (HDCP_DDC_WRITE_MAX_BYTE_NUM - 1)) {
		pr_debug_once("%s: write size too big\n", __func__);
		return -ERANGE;
	}
	...
	u32 *pdata, u32 count)
{
	...
	if (hdcp_ctrl->tz_hdcp) {
		...
		while (count) {
			...
			if (ret || resp) {
				pr_debug_once("%s: error: scm_call ret=%d resp=%u\n",
					__func__, ret, resp);
				ret = -EINVAL;
				break;
			}
...
static int msm_hdmi_hdcp_read_validate_aksv(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	if ((hweight32(hdcp_ctrl->aksv_lsb) + hweight32(hdcp_ctrl->aksv_msb))
			!= 20) {
		pr_debug_once("%s: AKSV QFPROM doesn't have 20 1's, 20 0's\n",
			__func__);
		pr_debug_once("%s: QFPROM AKSV chk failed (AKSV=%02x%08x)\n",
...
static int msm_hdmi_hdcp_auth_prepare(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	if (!hdcp_ctrl->aksv_valid) {
		...
		if (rc) {
			pr_debug_once("%s: ASKV validation failed\n", __func__);
...
static int msm_hdmi_hdcp_wait_key_an_ready(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	do {
		...
		timeout_count--;
		if (!timeout_count) {
			pr_debug_once("%s: Wait key state timedout", __func__);
			return -ETIMEDOUT;
		}
	...
	do {
		...
		timeout_count--;
		if (!timeout_count) {
			pr_debug_once("%s: Wait An timedout", __func__);
			return -ETIMEDOUT;
		}
...
static int msm_hdmi_hdcp_send_aksv_an(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	rc = msm_hdmi_ddc_write(hdmi, HDCP_PORT_ADDR, 0x18, (u8 *)link0_an,
		(u16)sizeof(link0_an));
	if (rc) {
		pr_debug_once("%s:An write failed\n", __func__);
		return rc;
	}
	...
	if (rc) {
		pr_debug_once("%s:AKSV write failed\n", __func__);
		return rc;
	}
...
static int msm_hdmi_hdcp_recv_bksv(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	if (rc) {
		pr_debug_once("%s:BKSV read failed\n", __func__);
		return rc;
	}
	...
	if ((hweight32(hdcp_ctrl->bksv_lsb) + hweight32(hdcp_ctrl->bksv_msb))
			!= 20) {
		pr_debug_once(": BKSV doesn't have 20 1's and 20 0's\n");
		pr_debug_once(": BKSV chk fail. BKSV=%02x%02x%02x%02x%02x\n",
...
static int msm_hdmi_hdcp_recv_bcaps(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	if (rc) {
		pr_debug_once("%s:BCAPS read failed\n", __func__);
		return rc;
	}
	DBG("BCAPS=%02x", bcaps);
...
static int msm_hdmi_hdcp_auth_part1_key_exchange(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	if (rc) {
		pr_debug_once("%s: wait key and an ready failed\n", __func__);
		return rc;
	};
	...
	if (rc) {
		pr_debug_once("%s: read bcaps error, abort\n", __func__);
		return rc;
	}
	...
	if (rc) {
		pr_debug_once("%s:An/Aksv write failed\n", __func__);
		return rc;
	}
	...
	if (rc) {
		pr_debug_once("%s:BKSV Process failed\n", __func__);
		return rc;
	}
...
static int msm_hdmi_hdcp_auth_part1_recv_r0(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	if (rc) {
		pr_debug_once("%s:R0' read failed\n", __func__);
		return rc;
	}
...
static int msm_hdmi_hdcp_auth_part1_verify_r0(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	if (!rc) {
		pr_debug_once("%s: Wait Auth IRQ timeout\n", __func__);
		return -ETIMEDOUT;
	}

	link0_status = hdmi_read(hdmi, REG_HDMI_HDCP_LINK0_STATUS);
	if (!(link0_status & HDMI_HDCP_LINK0_STATUS_RI_MATCHES)) {
		pr_debug_once("%s: Authentication Part I failed\n", __func__);
		return -EINVAL;
	}
	...
	u16 *pbstatus)
{
	...
	if (rc) {
		pr_debug_once("%s: BSTATUS read failed\n", __func__);
		goto error;
	}
	*pbstatus = bstatus = (buf[1] << 8) | buf[0];
	...
	if (down_stream_devices == 0) {
		 ...
		 */
		pr_debug_once("%s: No downstream devices\n", __func__);
		rc = -EINVAL;
		goto error;
	}
	...
	if (max_devs_exceeded) {
		pr_debug_once("%s: no. of devs connected exceeds max allowed",
			__func__);
		rc = -EINVAL;
		goto error;
	}
	...
	if (max_cascade_exceeded) {
		pr_debug_once("%s: no. of cascade conn exceeds max allowed",
			__func__);
		rc = -EINVAL;
		goto error;
	}
	...
	struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	do {
		...
		timeout_count--;
		if (!timeout_count) {
			pr_debug_once("%s: Wait KSV fifo ready timedout", __func__);
			return -ETIMEDOUT;
		}
	...
	rc = msm_hdmi_hdcp_scm_wr(hdcp_ctrl, &reg, &data, 1);
	if (rc) {
		pr_debug_once("%s: BSTATUS write failed\n", __func__);
		return rc;
	}
...
static int msm_hdmi_hdcp_recv_ksv_fifo(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	if (rc)
		pr_debug_once("%s: KSV FIFO read failed\n", __func__);
	...
	struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	do {
		...
		timeout_count--;
		if (!timeout_count) {
			pr_debug_once("%s: Recv ksv fifo timedout", __func__);
			return -ETIMEDOUT;
		}
	...
	if (rc) {
		pr_debug_once("%s: transfer V failed\n", __func__);
		return rc;
	}
	...
	if (rc) {
		pr_debug_once("%s: fail to reset sha engine\n", __func__);
		return rc;
	}
	...
	struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	do {
		...
		timeout_count--;
		if (!timeout_count) {
			pr_debug_once("%s: Write KSV fifo timedout", __func__);
			return -ETIMEDOUT;
		}
...
static int msm_hdmi_hdcp_auth_part2_check_v_match(struct hdmi_hdcp_ctrl *hdcp_ctrl)
{
	...
	do {
		...
		timeout_count--;
		if (!timeout_count) {
				pr_debug_once("%s: HDCP V Match timedout", __func__);
				return -ETIMEDOUT;
		}
...
static void msm_hdmi_hdcp_auth_work(struct work_struct *work)
{
	...
	if (rc) {
		pr_debug_once("%s: auth prepare failed %d\n", __func__, rc);
		goto end;
	}
	...
	if (rc) {
		pr_debug_once("%s: key exchange failed %d\n", __func__, rc);
		goto end;
	}

	rc = msm_hdmi_hdcp_auth_part1_recv_r0(hdcp_ctrl);
	if (rc) {
		pr_debug_once("%s: receive r0 failed %d\n", __func__, rc);
		goto end;
	}

	rc = msm_hdmi_hdcp_auth_part1_verify_r0(hdcp_ctrl);
	if (rc) {
		pr_debug_once("%s: verify r0 failed %d\n", __func__, rc);
		goto end;
	}
	...
	if (rc) {
		pr_debug_once("%s: wait ksv fifo ready failed %d\n", __func__, rc);
		goto end;
	}

	rc = msm_hdmi_hdcp_auth_part2_recv_ksv_fifo(hdcp_ctrl);
	if (rc) {
		pr_debug_once("%s: recv ksv fifo failed %d\n", __func__, rc);
		goto end;
	}

	rc = msm_hdmi_hdcp_auth_part2_write_ksv_fifo(hdcp_ctrl);
	if (rc) {
		pr_debug_once("%s: write ksv fifo failed %d\n", __func__, rc);
		goto end;
	}

	rc = msm_hdmi_hdcp_auth_part2_check_v_match(hdcp_ctrl);
	if (rc)
		pr_debug_once("%s: check v match failed %d\n", __func__, rc);
	...
	if (rc == -ECANCELED) {
		...
		pr_debug_once("%s: hdcp authentication failed\n", __func__);
...
struct hdmi_hdcp_ctrl *msm_hdmi_hdcp_init(struct hdmi *hdmi)
{
	...
	if (!hdmi->qfprom_mmio) {
		pr_debug_once("%s: HDCP is not supported without qfprom\n",
			__func__);
