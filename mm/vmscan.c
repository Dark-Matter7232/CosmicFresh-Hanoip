			  ...
			  struct mm_walk *args)
{
	...
	unsigned long addr;
	unsigned long next;
	struct lru_gen_mm_walk *walk = walk->private;
...
static void walk_mm(struct lruvec *lruvec, struct mm_struct *mm, struct lru_gen_mm_walk *walk)
{
	...
	do {
		...
		if (down_read_trylock(&mm->mmap_sem)) {
			err = walk_page_range(mm, walk->next_addr, ULONG_MAX, &args);
...
static unsigned long get_nr_to_scan(struct lruvec *lruvec, struct scan_control *sc, bool can_swap,
			    unsigned long reclaimed, bool *need_aging, unsigned long *evictable)
{
	int priority;
	unsigned long nr_to_scan;
	struct mem_cgroup *memcg = lruvec_memcg(lruvec);
...
static unsigned long lru_gen_shrink_lruvec(struct lruvec *lruvec, struct scan_control *sc)
{
	...
	while (true) {
		...
		nr_to_scan = get_nr_to_scan(lruvec, sc, swappiness, reclaimed, &need_aging, &evictable);
		if (!nr_to_scan)
			goto done;
