				     ...
				     struct dsi_regulator_info *regs)
{
	...
	dsi_for_each_child_node(root, node) {
		...
		if (rc) {
			pr_debug_once("failed to read name, rc = %d\n", rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("failed to read min voltage, rc = %d\n", rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("failed to read max voltage, rc = %d\n", rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("failed to read enable load, rc = %d\n", rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("failed to read disable load, rc = %d\n", rc);
			goto error;
		}
	...
	if (enable) {
		for (i = 0; i < regs->count; i++) {
			...
			rc = regulator_set_load(vreg->vreg,
						vreg->enable_load);
			if (rc < 0) {
				pr_debug_once("Setting optimum mode failed for %s\n",
				       vreg->vreg_name);
				goto error;
			}
			...
			if (num_of_v > 0) {
				rc = regulator_set_voltage(vreg->vreg,
							   ...
							   vreg->max_voltage);
				if (rc) {
					pr_debug_once("Set voltage(%s) fail, rc=%d\n",
						 vreg->vreg_name, rc);
			...
			if (rc) {
				pr_debug_once("enable failed for %s, rc=%d\n",
				       vreg->vreg_name, rc);
				 ...
				 char *supply_name)
{
	...
	if (regs->count == 0) {
		pr_debug_once("No vregs defined for %s\n", supply_name);
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug_once("failed to parse supply node for %s, rc = %d\n",
			supply_name, rc);
...
int dsi_pwr_enable_regulator(struct dsi_regulator_info *regs, bool enable)
{
	...
	if (enable) {
		...
		if (regs->refcount == 0) {
			pr_debug_once("Unbalanced regulator off:%s\n",
			...
			if (regs->refcount == 0) {
				...
				if (rc)
					pr_debug_once("failed to disable vregs\n");
			}
		}
	}
					 ...
					 int regulator_mode)
{
	...
	for (i = 0; i < regs->count; i++) {
		...
		if (!strcmp(vreg->vreg_name, reg_name)) {
			rc = regulator_set_mode(vreg->vreg,
						regulator_mode);
			if (rc)
				pr_debug_once("Regulator %s set mode %d failed\n",
				       vreg->vreg_name, rc);
			break;
		}
	}
	...
	if (i >= regs->count) {
		pr_debug_once("Regulator %s was not found\n", reg_name);
		return -EINVAL;
	}
