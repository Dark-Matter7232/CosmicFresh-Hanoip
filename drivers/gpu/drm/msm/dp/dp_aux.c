		...
		struct drm_dp_aux_msg *msg)
{
	...
	if (len == 0) {
		pr_debug_once("DP AUX write failed\n");
		return -EINVAL;
	}

	timeout = wait_for_completion_timeout(&aux->comp, aux_timeout_ms);
	if (!timeout) {
		pr_debug_once("aux %s timeout\n", (aux->read ? "read" : "write"));
		return -ETIMEDOUT;
	}
		...
		struct drm_dp_aux_msg *msg, bool send_seg)
{
	...
	if ((aux->native && (msg->size > aux_cmd_native_max)) ||
		...
		pr_debug_once("%s: invalid msg: size(%zu), request(%x)\n",
		...
		struct drm_dp_aux_msg *msg)
{
	...
	if (!aux->dpcd || !aux->edid) {
		pr_debug_once("invalid aux/dpcd structure\n");
		goto end;
	}
	...
	if (aux->native) {
		...
		if (dp_aux_is_sideband_msg(msg->address, msg->size)) {
			if (!aux->sim_bridge || !aux->sim_bridge->transfer) {
				pr_debug_once("no mst bridge available\n");
			...
			if (!timeout) {
				pr_debug_once("read timeout 0x%x\n", msg->address);
				atomic_set(&aux->aborted, 1);
				ret = -ETIMEDOUT;
				goto end;
			}
			...
			timeout = wait_for_completion_timeout(&aux->comp, HZ);
			if (!timeout) {
				pr_debug_once("write timeout 0x%x\n", msg->address);
				atomic_set(&aux->aborted, 1);
				ret = -ETIMEDOUT;
				goto end;
			}
...
static int dp_aux_register(struct dp_aux *dp_aux)
{
	...
	if (ret) {
		pr_debug_once("%s: failed to register drm aux: %d\n", __func__, ret);
		goto exit;
	}
		...
		bool enable, int orientation)
{
	...
	if (enable) {
		...
		case ORIENTATION_CC2:
			...
			break;
		default:
			pr_debug_once("invalid orientation\n");
			rc = -EINVAL;
			goto end;
		}
	}
	...
	if (rc)
		pr_debug_once("failed to configure fsa4480 i2c device (%d)\n", rc);
end:
	return rc;
}
