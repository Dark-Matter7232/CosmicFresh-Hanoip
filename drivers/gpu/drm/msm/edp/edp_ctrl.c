...
static int edp_clk_init(struct edp_ctrl *ctrl)
{
	...
	if (IS_ERR(ctrl->aux_clk)) {
		...
		pr_debug_once("%s: Can't find aux_clk, %d\n", __func__, ret);
		ctrl->aux_clk = NULL;
		return ret;
	}
	...
	if (IS_ERR(ctrl->pixel_clk)) {
		...
		pr_debug_once("%s: Can't find pixel_clk, %d\n", __func__, ret);
		ctrl->pixel_clk = NULL;
		return ret;
	}
	...
	if (IS_ERR(ctrl->ahb_clk)) {
		...
		pr_debug_once("%s: Can't find ahb_clk, %d\n", __func__, ret);
		ctrl->ahb_clk = NULL;
		return ret;
	}
	...
	if (IS_ERR(ctrl->link_clk)) {
		...
		pr_debug_once("%s: Can't find link_clk, %d\n", __func__, ret);
		ctrl->link_clk = NULL;
		return ret;
	}
	...
	if (IS_ERR(ctrl->mdp_core_clk)) {
		...
		pr_debug_once("%s: Can't find mdp_core_clk, %d\n", __func__, ret);
...
static int edp_clk_enable(struct edp_ctrl *ctrl, u32 clk_mask)
{
	...
	if (clk_mask & EDP_CLK_MASK_AHB) {
		...
		if (ret) {
			pr_debug_once("%s: Failed to enable ahb clk\n", __func__);
			goto f0;
		}
	}
	if (clk_mask & EDP_CLK_MASK_AUX) {
		...
		if (ret) {
			pr_debug_once("%s: Failed to set rate aux clk\n", __func__);
			goto f1;
		}
		ret = clk_prepare_enable(ctrl->aux_clk);
		if (ret) {
			pr_debug_once("%s: Failed to enable aux clk\n", __func__);
			goto f1;
		}
	}
	...
	if (clk_mask & EDP_CLK_MASK_LINK) {
		...
		ret = clk_set_rate(ctrl->link_clk,
				(unsigned long)ctrl->link_rate * 27000000);
		if (ret) {
			pr_debug_once("%s: Failed to set rate to link clk\n",
				__func__);
			goto f2;
		}
		...
		if (ret) {
			pr_debug_once("%s: Failed to enable link clk\n", __func__);
			goto f2;
		}
	}
	if (clk_mask & EDP_CLK_MASK_PIXEL) {
		...
		ret = clk_set_rate(ctrl->pixel_clk,
				(unsigned long)ctrl->pixel_rate * 1000);
		if (ret) {
			pr_debug_once("%s: Failed to set rate to pixel clk\n",
				__func__);
			goto f3;
		}
		...
		if (ret) {
			pr_debug_once("%s: Failed to enable pixel clk\n", __func__);
			goto f3;
		}
	}
	if (clk_mask & EDP_CLK_MASK_MDP_CORE) {
		...
		if (ret) {
			pr_debug_once("%s: Failed to enable mdp core clk\n", __func__);
			goto f4;
		}
	}
...
static int edp_regulator_init(struct edp_ctrl *ctrl)
{
	...
	if (ret) {
		pr_debug_once("%s: Could not get vdda reg, ret = %d\n", __func__,
				ret);
		ctrl->vdda_vreg = NULL;
		return ret;
	}
	...
	if (ret) {
		pr_debug_once("%s: Could not get lvl-vdd reg, ret = %d\n", __func__,
				ret);
		ctrl->lvl_vreg = NULL;
		return ret;
	}
...
static int edp_regulator_enable(struct edp_ctrl *ctrl)
{
	...
	if (ret < 0) {
		pr_debug_once("%s: vdda_vreg set regulator mode failed.\n", __func__);
		goto vdda_set_fail;
	}

	ret = regulator_enable(ctrl->vdda_vreg);
	if (ret) {
		pr_debug_once("%s: Failed to enable vdda_vreg regulator.\n", __func__);
		goto vdda_enable_fail;
	}

	ret = regulator_enable(ctrl->lvl_vreg);
	if (ret) {
		pr_debug_once("Failed to enable lvl-vdd reg regulator, %d", ret);
...
static int edp_gpio_config(struct edp_ctrl *ctrl)
{
	...
	if (IS_ERR(ctrl->panel_hpd_gpio)) {
		...
		pr_debug_once("%s: cannot get panel-hpd-gpios, %d\n", __func__, ret);
		return ret;
	}
	...
	if (IS_ERR(ctrl->panel_en_gpio)) {
		...
		pr_debug_once("%s: cannot get panel-en-gpios, %d\n", __func__, ret);
		return ret;
	}
	...
	u8 voltage_level, u8 pre_emphasis_level)
{
	...
	if (drm_dp_dpcd_write(ctrl->drm_aux, 0x103, buf, 4) < 4) {
		pr_debug_once("%s: Set sw/pe to panel failed\n", __func__);
		return -ENOLINK;
	}
...
static int edp_train_pattern_set_write(struct edp_ctrl *ctrl, u8 pattern)
{
	...
	if (drm_dp_dpcd_write(ctrl->drm_aux,
		...
		pr_debug_once("%s: Set training pattern to panel failed\n", __func__);
		return -ENOLINK;
	}
...
static void edp_host_train_set(struct edp_ctrl *ctrl, u32 train)
{
	...
	if (cnt == 0)
		pr_debug_once("%s: set link_train=%d failed\n", __func__, train);
}
...
static int edp_start_link_train_1(struct edp_ctrl *ctrl)
{
	...
	while (1) {
		...
		if (rlen < DP_LINK_STATUS_SIZE) {
			pr_debug_once("%s: read link status failed\n", __func__);
			return -ENOLINK;
		}
...
static int edp_do_link_train(struct edp_ctrl *ctrl)
{
	...
	if (ret < 0) {
		if (edp_link_rate_down_shift(ctrl) == 0) {
			...
			pr_debug_once("%s: Training 1 failed", __func__);
			ret = EDP_TRAIN_FAIL;
			goto clear;
		}
	}
	...
	if (ret < 0) {
		if (edp_link_rate_down_shift(ctrl) == 0) {
			...
			pr_debug_once("%s: Training 2 failed", __func__);
			ret = EDP_TRAIN_FAIL;
			goto clear;
		}
	}
...
static int edp_sw_mvid_nvid(struct edp_ctrl *ctrl, u32 m, u32 n)
{
	...
	if (ctrl->link_rate == DP_LINK_BW_1_62) {
		...
		pr_debug_once("%s: Invalid link rate, %d\n", __func__,
			ctrl->link_rate);
		return -EINVAL;
	}
...
int msm_edp_ctrl_init(struct msm_edp *edp)
{
	...
	if (ret) {
		pr_debug_once("%s:regulator init fail\n", __func__);
		return ret;
	}
	ret = edp_clk_init(ctrl);
	if (ret) {
		pr_debug_once("%s:clk init fail\n", __func__);
		return ret;
	}
	ret = edp_gpio_config(ctrl);
	if (ret) {
		pr_debug_once("%s:failed to configure GPIOs: %d", __func__, ret);
		return ret;
	}
	...
	if (!ctrl->aux || !ctrl->drm_aux) {
		pr_debug_once("%s:failed to init aux\n", __func__);
		return -ENOMEM;
	}

	ctrl->phy = msm_edp_phy_init(dev, ctrl->base);
	if (!ctrl->phy) {
		pr_debug_once("%s:failed to init phy\n", __func__);
		ret = -ENOMEM;
...
bool msm_edp_ctrl_panel_connected(struct edp_ctrl *ctrl)
{
	...
	if (drm_dp_dpcd_read(ctrl->drm_aux, DP_DPCD_REV, ctrl->dpcd,
		...
		pr_debug_once("%s: AUX channel is NOT ready\n", __func__);
		...
		struct drm_connector *connector, struct edid **edid)
{
	...
	if (ret) {
		pr_debug_once("%s: read dpcd cap failed, %d\n", __func__, ret);
		goto disable_ret;
	}
	...
	if (!ctrl->edid) {
		pr_debug_once("%s: edid read fail\n", __func__);
		goto disable_ret;
	}
				...
				const struct drm_display_info *info)
{
	...
	if (edp_clk_enable(ctrl, EDP_CLK_MASK_AHB)) {
		pr_debug_once("%s, fail to prepare enable ahb clk\n", __func__);
		ret = -EINVAL;
		goto unlock_ret;
	}
