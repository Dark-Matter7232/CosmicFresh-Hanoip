...
static int max3108_detect(struct device *dev)
{
	...
	if (ret){
		dev_err(dev, "%s regmap_read error %d\n", __func__, ret);
		return ret;
	}
				...
				struct ktermios *old)
{
	...
	pr_debug("%s cflag 0x%x iflag 0x%x flow 0x%x\n", __func__,
		termios->c_cflag, termios->c_iflag, flow);

	if (flow & MAX310X_FLOWCTRL_AUTOCTS_BIT) {
		max310x_port_update(port, MAX310X_MODE1_REG,
...
static int max310x_of_init(struct device *dev)
{
	...
	s->use_ext_clock = of_property_read_bool(node, "max,use-ext-clock");
	of_property_read_s32(node, "max,ext-clock-freq", &s->ext_clock_freq);

	if (!IS_ERR(s->vcc_supply)) {
		prop = of_get_property(node, "max,vcc-voltage-level", &len);
		if (!prop || (len != (2 * sizeof(__be32)))) {
			dev_err(dev, "supply voltage levels not specified len %d\n", len);

			supply_vol_low = MAX310X_VCC_DEFAULT;
			supply_vol_high = MAX310X_VCC_DEFAULT;
		} else {
		...
		dev_info(dev, "%s set vcc supply as %d %d\n",
				__func__, supply_vol_low, supply_vol_high);
		regulator_set_voltage(s->vcc_supply, supply_vol_low, supply_vol_high);

		ret = regulator_set_load(s->vcc_supply, 100000);
		if (ret ) {
			dev_err(dev, "Failed to set reg vcc_supply cur: %d\n", ret);
		}

		ret = regulator_enable(s->vcc_supply);
		if (ret) {
			dev_err(dev, "Unable to enable vcc_supply\n");
			return ret;
		}
	}

	//reset
	s->gpio_reset = of_get_named_gpio(node, "max,gpio-reset", 0);
	if (s->gpio_reset < 0) {
		pr_err("failed to get max,gpio_reset-gpio.\n");
		ret = s->gpio_reset;
		goto end;
	}

	if (gpio_is_valid(s->gpio_reset)) {
	...
	//ldo-en
	s->gpio_ldo_en = of_get_named_gpio(node, "max,gpio-ldo-en", 0);
	if (s->gpio_ldo_en < 0) {
		pr_err("failed to get max,ldo-en-gpio.\n");
		//s->gpio_ldo_en;
		goto end;
	}

	if (gpio_is_valid(s->gpio_ldo_en)) {
			 ...
			 struct regmap *regmap, int irq, unsigned long flags)
{
	...
	s->vcc_supply = devm_regulator_get(dev, "vcc");
	if (IS_ERR(s->vcc_supply))  {
		dev_info(dev, "there is no vcc supply\n");
	}

	if (max310x_of_init(dev)){
		dev_err(dev, "of dev init error\n");
		return -EINVAL;
	}

	if (s->ext_clock_freq > 0){
		freq = s->ext_clock_freq;
		dev_err(dev, "use fix ext clock, set freq as %d\n", freq);
		goto jump_internal_clock;
	}
...
static const struct of_device_id __maybe_unused max310x_dt_ids[] = {
	{ .compatible = "max,max3107",	.data = &max3107_devtype, },
	{ .compatible = "max,max3108",	.data = &max3108_devtype, },
	{ .compatible = "max,max3109",	.data = &max3109_devtype, },
	{ .compatible = "max,max14830",	.data = &max14830_devtype },
	{ }
};
...
static int max310x_i2c_probe(struct i2c_client *i2c,const struct i2c_device_id *id)
{
	...
	if (i2c->dev.of_node) {
		const struct of_device_id *of_id =
			of_match_device(max310x_dt_ids, &i2c->dev);

		devtype = (struct max310x_devtype *) of_id->data;
	} else {
		printk("device of node invalid\n");
		return -1;
	}
	printk("chiptype = %s \n", devtype->name);

	regcfg.max_register = devtype->nr * 0x20 - 1;
	regcfg.write_flag_mask = 0x00;
	regmap = devm_regmap_init_i2c(i2c, &regcfg);

	flags = IRQF_TRIGGER_FALLING;

	return max310x_probe(&i2c->dev, devtype, regmap,  i2c->irq, flags);
}
