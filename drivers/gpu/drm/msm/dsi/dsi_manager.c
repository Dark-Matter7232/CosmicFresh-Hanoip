	...
	if (!msm_dsi->panel) {
		...
		if (msm_dsi->panel && IS_DUAL_DSI() &&
			...
			if (kms->funcs->set_split_display)
				...
				pr_debug_once("mdp does not support dual DSI\n");
		}
	}
			...
			struct drm_connector *connector, int id)
{
	...
	if (!connector->tile_group)
		connector->tile_group = drm_mode_create_tile_group(
					connector->dev, topo_id);
	if (!connector->tile_group) {
		pr_debug_once("%s: failed to create tile group\n", __func__);
		return -ENOMEM;
	}
...
static int dsi_mgr_connector_get_modes(struct drm_connector *connector)
{
	...
	if (IS_DUAL_DSI()) {
		...
		if (ret) {
			pr_debug_once("%s: set tile property failed, %d\n",
					__func__, ret);
			return ret;
		}
	}
...
static void dsi_mgr_bridge_pre_enable(struct drm_bridge *bridge)
{
	...
	if (ret) {
		pr_debug_once("%s: power on host %d failed, %d\n", __func__, id, ret);
		goto host_on_fail;
	}
	...
	if (is_dual_dsi && msm_dsi1) {
		...
		if (ret) {
			pr_debug_once("%s: power on host1 failed, %d\n",
							__func__, ret);
			goto host1_on_fail;
		}
	}
	...
	if (panel) {
		...
		if (ret) {
			pr_debug_once("%s: prepare panel %d failed, %d\n", __func__,
								id, ret);
	...
	if (ret) {
		pr_debug_once("%s: enable host %d failed, %d\n", __func__, id, ret);
		goto host_en_fail;
	}
	...
	if (is_dual_dsi && msm_dsi1) {
		...
		if (ret) {
			pr_debug_once("%s: enable host1 failed, %d\n", __func__, ret);
			goto host1_en_fail;
		}
	}
	...
	if (panel) {
		...
		if (ret) {
			pr_debug_once("%s: enable panel %d failed, %d\n", __func__, id,
									ret);
			goto panel_en_fail;
		}
	}
...
static void dsi_mgr_bridge_post_disable(struct drm_bridge *bridge)
{
	...
	if (panel) {
		...
		if (ret)
			pr_debug_once("%s: Panel %d OFF failed, %d\n", __func__, id,
									ret);
	}

	ret = msm_dsi_host_disable(host);
	if (ret)
		pr_debug_once("%s: host %d disable failed, %d\n", __func__, id, ret);

	if (is_dual_dsi && msm_dsi1) {
		ret = msm_dsi_host_disable(msm_dsi1->host);
		if (ret)
			pr_debug_once("%s: host1 disable failed, %d\n", __func__, ret);
	}

	if (panel) {
		ret = drm_panel_unprepare(panel);
		if (ret)
			pr_debug_once("%s: Panel %d unprepare failed,%d\n", __func__,
								id, ret);
	}
	...
	if (ret)
		pr_debug_once("%s: host %d power off failed,%d\n", __func__, id, ret);

	if (is_dual_dsi && msm_dsi1) {
		ret = msm_dsi_host_power_off(msm_dsi1->host);
		if (ret)
			pr_debug_once("%s: host1 power off failed, %d\n",
								__func__, ret);
	}
...
int msm_dsi_manager_cmd_xfer(int id, const struct mipi_dsi_msg *msg)
{
	...
	if (need_sync && msm_dsi0) {
		...
		if (ret) {
			pr_debug_once("%s: failed to prepare non-trigger host, %d\n",
				__func__, ret);
			return ret;
		}
	}
	...
	if (ret) {
		pr_debug_once("%s: failed to prepare host, %d\n", __func__, ret);
		goto restore_host0;
	}
...
int msm_dsi_manager_register(struct msm_dsi *msm_dsi)
{
	...
	if (id > DSI_MAX) {
		pr_debug_once("%s: invalid id %d\n", __func__, id);
		return -EINVAL;
	}

	if (msm_dsim->dsi[id]) {
		pr_debug_once("%s: dsi%d already registered\n", __func__, id);
		return -EBUSY;
	}
	...
	if (ret) {
		pr_debug_once("%s: failed to parse dual DSI info\n", __func__);
		goto fail;
	}

	ret = dsi_mgr_setup_components(id);
	if (ret) {
		pr_debug_once("%s: failed to register mipi dsi host for DSI %d\n",
			__func__, id);
		goto fail;
	}
