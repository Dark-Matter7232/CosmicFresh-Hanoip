...
static struct dp_audio_private *dp_audio_get_data(struct platform_device *pdev)
{
	...
	if (!ext_data) {
		pr_debug_once("invalid ext disp data\n");
		return ERR_PTR(-EINVAL);
	}

	dp_audio = ext_data->intf_data;
	if (!ext_data) {
		pr_debug_once("invalid intf data\n");
		return ERR_PTR(-EINVAL);
	}
...
static int dp_audio_register_ext_disp(struct dp_audio_private *audio)
{
	...
	if (!audio->pdev->dev.of_node) {
		pr_debug_once("cannot find audio dev.of_node\n");
		rc = -ENODEV;
		goto end;
	}
	...
	if (!pd) {
		pr_debug_once("cannot parse %s handle\n", phandle);
		rc = -ENODEV;
		goto end;
	}
	...
	if (!audio->ext_pdev) {
		pr_debug_once("cannot find %s pdev\n", phandle);
		rc = -ENODEV;
		goto end;
	}
	...
	if (rc)
		pr_debug_once("failed to register disp\n");
end:
	if (pd)
		of_node_put(pd);
...
static int dp_audio_deregister_ext_disp(struct dp_audio_private *audio)
{
	...
	if (rc)
		pr_debug_once("failed to deregister disp\n");
...
static int dp_audio_notify(struct dp_audio_private *audio, u32 state)
{
	...
	if (!ext->intf_ops.audio_notify) {
		pr_debug_once("audio notify not defined\n");
		goto end;
	}
	...
	if (!rc) {
		pr_debug_once("timeout. state=%d err=%d\n", state, rc);
		rc = -ETIMEDOUT;
		goto end;
	}
...
static int dp_audio_config(struct dp_audio_private *audio, u32 state)
{
	...
	if (!ext || !ext->intf_ops.audio_config) {
		pr_debug_once("audio_config not defined\n");
		goto end;
	}
	...
	if (audio->panel->stream_id == DP_STREAM_0) {
		rc = ext->intf_ops.audio_config(audio->ext_pdev,
				&ext->codec, state);
		if (rc)
			pr_debug_once("failed to config audio, err=%d\n", rc);
	}
end:
	return rc;
}
...
static int dp_audio_create_notify_workqueue(struct dp_audio_private *audio)
{
	...
	if (IS_ERR_OR_NULL(audio->notify_workqueue)) {
		pr_debug_once("Error creating notify_workqueue\n");
		return -EPERM;
	}
