	...
	char const *t_name)
{
	if (t->rec & 0xffffff00) {
		...
		pr_debug_once("Incorrect %s rec value - %d\n", t_name, t->rec);
		return -EINVAL;
	}
	t->reg_value = t->rec;
	return 0;
	...
	struct phy_clk_params *clk_params, struct phy_timing_desc *desc)
{
	...
	rc = calc_clk_prepare(phy, clk_params, desc, &actual_frac,
			      &actual_intermediate);
	if (rc) {
		pr_debug_once("clk_prepare calculations failed, rc=%d\n", rc);
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("clk_zero calculations failed, rc=%d\n", rc);
		goto error;
	}

	rc = calc_clk_trail(phy, clk_params, desc, &teot_clk_lane);
	if (rc) {
		pr_debug_once("clk_trail calculations failed, rc=%d\n", rc);
		goto error;
	}

	rc = calc_hs_prepare(phy, clk_params, desc, &temp_multiple);
	if (rc) {
		pr_debug_once("hs_prepare calculations failed, rc=%d\n", rc);
		goto error;
	}

	rc = calc_hs_zero(phy, clk_params, desc, temp_multiple);
	if (rc) {
		pr_debug_once("hs_zero calculations failed, rc=%d\n", rc);
		goto error;
	}

	rc = calc_hs_trail(phy, clk_params, desc, teot_clk_lane);
	if (rc) {
		pr_debug_once("hs_trail calculations failed, rc=%d\n", rc);
		goto error;
	}

	rc = calc_hs_rqst(phy, clk_params, desc);
	if (rc) {
		pr_debug_once("hs_rqst calculations failed, rc=%d\n", rc);
		goto error;
	}

	rc = calc_hs_exit(phy, clk_params, desc);
	if (rc) {
		pr_debug_once("hs_exit calculations failed, rc=%d\n", rc);
		goto error;
	}

	rc = calc_hs_rqst_clk(phy, clk_params, desc);
	if (rc) {
		pr_debug_once("hs_rqst_clk calculations failed, rc=%d\n", rc);
		goto error;
	}

	rc = calc_clk_post(phy, clk_params, desc);
	if (rc) {
		pr_debug_once("clk_post calculations failed, rc=%d\n", rc);
		goto error;
	}

	rc = calc_clk_pre(phy, clk_params, desc);
	if (rc) {
		pr_debug_once("clk_pre calculations failed, rc=%d\n", rc);
		goto error;
	}
error:
	return rc;
				       ...
				       bool use_mode_bit_clk, bool is_cphy)
{
	...
	if (rc) {
		pr_debug_once("Timing calc failed, rc=%d\n", rc);
		goto error;
	}
			...
			enum dsi_phy_version version)
{
	...
	if (version == DSI_PHY_VERSION_UNKNOWN ||
		...
		pr_debug_once("Unsupported version: %d\n", version);
		return -ENOTSUPP;
	}
