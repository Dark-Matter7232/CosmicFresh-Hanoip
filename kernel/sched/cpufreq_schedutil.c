...
static cpumask_t sched_governor_mask = CPU_MASK_NONE;
...
static bool check_sg_policy_initialized(void)
{
	...
	if (cpumask_weight(&sched_governor_mask) != CPU_NUM)
		return false;
...
int pmu_poll_init(void)
{
	...
	ret = sched_setscheduler_nocheck(thread, SCHED_FIFO, &sched_max_rt_prio);
	if (ret) {
		kthread_stop(thread);
...
static void sugov_exit(struct cpufreq_policy *policy)
{
	...
	cpumask_andnot(&sched_governor_mask, &sched_governor_mask, policy->cpus);
...
static int sugov_start(struct cpufreq_policy *policy)
{
	...
	cpumask_or(&sched_governor_mask, &sched_governor_mask, policy->cpus);
