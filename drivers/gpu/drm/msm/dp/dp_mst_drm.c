				  ...
				  struct drm_display_mode *adjusted_mode)
{
	...
	if (IS_ERR(bridge_state)) {
		pr_debug_once("Invalid bridge state\n");
		ret = false;
		goto end;
	}

	if (!bridge_state->dp_panel) {
		pr_debug_once("Invalid dp_panel\n");
		ret = false;
		goto end;
	}
		...
		struct dp_display_mode *mode)
{
	...
	if (slots < 0) {
		pr_debug_once("mst: failed to find vcpi slots. pbn:%d, slots:%d\n",
				pbn, slots);
		return slots;
	}
...
static void _dp_mst_bridge_pre_enable_part1(struct dp_mst_bridge *dp_bridge)
{
	...
	ret = mst->mst_fw_cbs->allocate_vcpi(&mst->mst_mgr,
				       port, pbn, slots);
	if (ret == false) {
		pr_debug_once("mst: failed to allocate vcpi. bridge:%d\n",
				dp_bridge->id);
		return;
	}
...
static void dp_mst_bridge_pre_enable(struct drm_bridge *drm_bridge)
{
	...
	if (rc) {
		pr_debug_once("[%d] DP display prepare failed, rc=%d\n",
		       bridge->id, rc);
		goto end;
	}
	...
	if (rc) {
		pr_debug_once("[%d] DP display enable failed, rc=%d\n",
		       bridge->id, rc);
...
static void dp_mst_bridge_enable(struct drm_bridge *drm_bridge)
{
	...
	if (rc) {
		pr_debug_once("mst bridge [%d] post enable failed, rc=%d\n",
		       bridge->id, rc);
		return;
	}
...
static void dp_mst_bridge_disable(struct drm_bridge *drm_bridge)
{
	...
	if (rc)
		pr_debug_once("[%d] DP display pre disable failed, rc=%d\n",
		       bridge->id, rc);
...
int dp_mst_drm_bridge_init(void *data, struct drm_encoder *encoder)
{
	...
	if (i == MAX_DP_MST_DRM_BRIDGES) {
		pr_debug_once("mst supports only %d bridges\n", i);
		rc = -EACCES;
		goto end;
	}
	...
	if (!rc) {
		...
		if (bridge->fixed_connector == NULL) {
			pr_debug_once("failed to create fixed connector\n");
			kfree(state);
			rc = -ENOMEM;
			goto end;
		}
	...
	if (drm_atomic_crtc_needs_modeset(crtc_state)) {
		...
		if (slots > 0) {
			rc = mst->mst_fw_cbs->atomic_release_vcpi_slots(state,
				&mst->mst_mgr, slots);
			if (rc) {
				pr_debug_once("failed releasing %d vcpi slots %d\n",
						slots, rc);
				goto end;
			}
		}
		...
		struct drm_dp_mst_port *port, const char *pathprop)
{
	...
	if (!connector) {
		pr_debug_once("mst sde_connector_init failed\n");
	...
	if (rc) {
		pr_debug_once("mst connector install failed\n");
...
static void dp_mst_display_hpd_irq(void *dp_display)
{
	...
	if (!mst->mst_session_state) {
		pr_debug_once("mst_hpd_irq received before mst session start\n");
		return;
	}
	...
	rc = drm_dp_dpcd_read(mst->caps.drm_aux, DP_SINK_COUNT_ESI,
		esi, 14);
	if (rc != 14) {
		pr_debug_once("dpcd sink status read failed, rlen=%d\n", rc);
		return;
	}
	...
	if (handled) {
		...
		if (rc != 3)
			pr_debug_once("dpcd esi_res failed. rlen=%d\n", rc);
	}
...
int dp_mst_init(struct dp_display *dp_display)
{
	...
	ret = drm_dp_mst_topology_mgr_init(&dp_mst.mst_mgr, dev,
					...
					conn_base_id);
	if (ret) {
		pr_debug_once("dp drm mst topology manager init failed\n");
		goto error;
	}
